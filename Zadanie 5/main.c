...
int main(void) {
	...
	while (is_running) {
		...
		printf("10. Display list back (off pointers)\n");
		printf("11. Display list back recuretion (off pointers)\n");
		printf("0. Exit \n");
		scanf("%d", &choice);
		...
		case 9:
			...
			break;
		case 10:
			head = display_behind(head);
			display_the_contents_of_the_list(head);
			break;
		case 11:
			head = display_behind_recurention(head);
			display_the_contents_of_the_list(head);
			break;
		case 0:
			is_running= false;
